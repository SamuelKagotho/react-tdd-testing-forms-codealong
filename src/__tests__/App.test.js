...
test("'Your Selection' message initially displays 'small cheese'", () => {
  ...
  expect(screen.getByText(/small\s+cheese/i)).toBeInTheDocument();
});
...
test("selecting options updates the 'Your selection' message", () => {
  ...
  expect(screen.getByText(/small\s+pepperoni/i)).toBeInTheDocument();

  userEvent.selectOptions(selectSize, "large");

  expect(screen.getByText(/large\s+pepperoni/i)).toBeInTheDocument();
});
